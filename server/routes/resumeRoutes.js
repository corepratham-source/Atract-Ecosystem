...
const createTFIDFVector = (text, vocab, idf) => {
  ...
  const tf = calculateTF(tokens, vocab);
  const vector = [];
...
// Upload and store resume (FREE - no OpenAI needed)
router.post("/upload", async (req, res) => {
  try {
    ...
    // Extract keywords from resume for faster matching
    const techKeywords = extractTechnicalKeywords(text);
    ...
    if (mongoose.connection.readyState === 1) {
      const savedResume = await Resume.create({
        ...
        email: email || "",
        text,
        techKeywords: Object.keys(techKeywords),
        softSkills: Object.keys(softSkills),
        embedding: null // No longer using OpenAI embeddings
      });
      ...
      return res.json({ 
        ...
        mode: "tfidf",
        extractedTechKeywords: Object.keys(techKeywords),
        extractedSoftSkills: Object.keys(softSkills)
      });
    } else {
...
// Match job description against stored resumes
...
// Direct resume analysis (single JD vs single resume)
...
router.get("/", async (req, res) => {
  try {
    const resumes = await Resume.find().select("name email createdAt");
    res.json(resumes);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
