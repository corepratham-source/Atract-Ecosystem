...
const ads = [
  { title: "🚀 Upgrade to Pro", text: "Get unlimited exit interview analyses per month", button: "Start Free Trial" },
  { title: "📊 HR Analytics Bundle", text: "Combine with Performance Review for insights", button: "Learn More" },
  { title: "🎯 Better Retention", text: "Identify turnover patterns before it's too late", button: "See How" },
];
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 ml-80 min-h-screen overflow-y-auto">
        <div className="p-6 max-w-6xl mx-auto">
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden relative">
                ...
                <div className="p-6 text-center border-b border-gray-100">
                  <div className="text-3xl mb-2">🔒</div>
                  <h2 className="text-2xl font-bold text-gray-900">Free Trials Used</h2>
                  <p className="text-gray-600 mt-1">Upgrade for unlimited analyses</p>
                </div>
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{app?.name || "Exit Interview Analyzer"}</h1>
            <p className="text-gray-600 mt-1">Get AI-powered insights from exit interviews</p>
            {!isPaid && (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        ...
                        value={form.employeeName}
                        onChange={(e) => setForm({ ...form, employeeName: e.target.value })}
                        className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none"
                        placeholder="John Doe"
                      />
                    </div>
                    <div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6">
                  ...
                  <ul className="space-y-2 text-sm text-gray-600">
                    <li>• Be specific about reasons for leaving</li>
                    <li>• Include quotes from the employee</li>
                    <li>• Mention manager feedback if available</li>
                    <li>• Note any compensation or growth issues</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          {activeTab === "results" && analysis && (
            <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Exit Interview Analysis</h2>
                  <p className="text-gray-600 mt-1">
                    {analysis.employeeName} • {analysis.department || "N/A"} • {analysis.role || "N/A"}
                  </p>
                </div>
                <button
                  onClick={() => {
...
Tenure: ${analysis.tenure || "N/A"}
Exit Reason: ${analysis.exitReason || "N/A"}
Generated: ${analysis.generatedAt || new Date().toLocaleString()}
...
${analysis.actions?.map(a => `[${a.priority}] ${a.action} (${a.timeline})`).join("\n") || "No actions recommended"}`;
              ...
              <div className="bg-gray-50 rounded-xl p-5 mb-6">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div>
                    <span className="text-xs text-gray-500">Employee</span>
                    <p className="font-medium">{analysis.employeeName || "N/A"}</p>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500">Department</span>
                    <p className="font-medium">{analysis.department || "N/A"}</p>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500">Tenure</span>
                    <p className="font-medium">{analysis.tenure || "N/A"}</p>
                  </div>
                  <div>
                    <span className="text-xs text-gray-500">Risk Level</span>
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${getRiskColor(analysis.retentionRisk)}`}>
                      {analysis.retentionRisk}
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className="bg-gray-50 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">Sentiment</h3>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`font-bold text-lg ${getSentimentColor(analysis.sentiment)}`}>
                      {analysis.sentiment}
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">Retention Risk</h3>
                  <span className={`inline-block px-4 py-2 rounded-lg font-bold text-lg ${getRiskColor(analysis.retentionRisk)}`}>
                    {analysis.retentionRisk} Risk
                  </span>
                </div>
              </div>
              ...
              {analysis.themes?.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">Key Themes</h3>
              ...
              {analysis.keywords?.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">Keywords</h3>
                  <div className="flex flex-wrap gap-2">
                    {analysis.keywords.map((kw, i) => (
                      <span key={i} className="px-3 py-1 bg-purple-100 text-purple-800 rounded-lg text-sm">
                        {kw}
                      </span>
                    ))}
                  </div>
              ...
              {analysis.actions?.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">Recommended Actions</h3>
                  <div className="space-y-3">
                    {analysis.actions.map((action, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                        <span className={`px-3 py-1 rounded text-xs font-bold flex-shrink-0 ${
                          action.priority === "High" ? "bg-red-100 text-red-700" :
                          action.priority === "Medium" ? "bg-amber-100 text-amber-700" :
                          "bg-green-100 text-green-700"
                        }`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">Timeline: {action.timeline}</p>
                        </div>
                      </div>
                    ))}
                  </div>
