...
const policyTypes = [
  { id: "leave", name: "Leave Policy" },
  { id: "workfromhome", name: "Work from Home Policy" },
  { id: "codeofconduct", name: "Code of Conduct" },
  { id: "recruitment", name: "Recruitment Policy" },
  { id: "performance", name: "Performance Review Policy" },
  { id: "disciplinary", name: "Disciplinary Policy" },
  { id: "confidentiality", name: "Confidentiality Policy" },
  { id: "grievance", name: "Grievance Redressal Policy" },
];

const companySizes = [
  { id: "1-20", name: "1-20 employees (Startup)" },
  { id: "21-100", name: "21-100 employees (SMB)" },
  { id: "101-500", name: "101-500 employees (Mid-size)" },
  { id: "500+", name: "500+ employees (Enterprise)" },
];

const tones = [
  { id: "formal", name: "Formal", desc: "Professional and authoritative" },
  { id: "neutral", name: "Neutral", desc: "Balanced and objective" },
  { id: "warm", name: "Warm", desc: "Friendly and approachable" },
];
...
export default function HRPolicyBuilder({ app = defaultApp }) {
  ...
  const generatePolicy = async () => {
    ...
    try {
      ...
      setOutput(data.policy || data.output || "");
      setActiveTab("results");
      if (!isPaid) updateTrialCount(trialCount + 1);
    } catch (err) {
      setError(err.message || "Something went wrong");
  ...
  const downloadAsPDF = () => {
    ...
    // Title
    doc.setFontSize(20);
    doc.text("HR Policy Document", pageWidth / 2, y, { align: "center" });
    y += 15;

    // Company Info
    doc.setFontSize(12);
    doc.text(`Company: ${form.companyName}`, 20, y);
    y += 8;
    doc.text(`Policy: ${policyTypes.find(p => p.id === form.policyType)?.name}`, 20, y);
    y += 15;
    ...
    // Footer
    doc.setFontSize(10);
    doc.text(`Generated on ${new Date().toLocaleDateString("en-IN")}`, 20, 280);

    doc.save(`${form.companyName.replace(/\s+/g, "_")}_Policy.pdf`);
  };
      ...
      <div className="flex-1 ml-80 min-h-screen flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            {!isPaid && (
              <div className="bg-amber-50 border border-amber-200 px-4 py-2 rounded-lg text-sm">
                Free trials left: <span className="font-semibold">{MAX_FREE_TRIALS - trialCount}</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-6 py-2.5 rounded-xl font-medium transition-all ${
                activeTab === "input" ? "bg-slate-900 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
            >
              📝 Configure Policy
            </button>
            <button
              ...
              disabled={!output}
              className={`px-6 py-2.5 rounded-xl font-medium transition-all ${
                activeTab === "results" ? "bg-slate-900 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === "input" && (
            <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
              <SectionTitle title="Build Your HR Policy" description="Fill details to generate a professional policy" />
              ...
              <div className="mt-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Field label="Company Name *">
                    <input
                      value={form.companyName}
                      onChange={(e) => setForm({ ...form, companyName: e.target.value })}
                      placeholder="ATRact Technologies Pvt Ltd"
                ...
                <Field label="Custom Clauses (Optional)">
                  <textarea
                    value={form.customClauses}
                    onChange={(e) => setForm({ ...form, customClauses: e.target.value })}
                    placeholder="Any specific rules you want to add..."
                    rows={3}
                    className="w-full rounded-2xl border border-slate-300 px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                  />
                </Field>
                ...
                <button
                  onClick={handleGenerate}
                  disabled={isLoading || !form.companyName.trim()}
                  className={`w-full py-4 rounded-2xl text-lg font-semibold transition-all ${
                    isLoading || !form.companyName.trim()
                      ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                      : "bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg"
                  }`}
                >
                  {isLoading ? "Generating Professional Policy..." : "🚀 Generate HR Policy"}
                </button>
              </div>
            </div>
          )}
          ...
          {activeTab === "results" && output && (
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
              <div className="flex justify-between items-center mb-6">
                <SectionTitle title="Generated HR Policy" description="Review and download your document" />
                <div className="flex gap-3">
                  <button
                    onClick={copyToClipboard}
                    className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 rounded-2xl font-medium flex items-center gap-2"
                  >
                    📋 Copy
                  </button>
                  <button
                    onClick={downloadAsPDF}
                    className="px-5 py-2.5 bg-indigo-600 text-white hover:bg-indigo-700 rounded-2xl font-medium flex items-center gap-2"
                  >
                    ⬇️ Download PDF
                  </button>
                </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 min-h-[600px] text-slate-800 leading-relaxed">
                <pre className="whitespace-pre-wrap font-serif text-[15.2px]">{output}</pre>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full">
            <div className="p-8 text-center">
              ...
              <h2 className="text-2xl font-bold">Free Trials Exhausted</h2>
              <p className="text-gray-600 mt-2">Unlock unlimited policy generation for ₹999</p>
            </div>
            <div className="px-8 pb-8">
              <button
                onClick={() => {/* your payment logic */}}
                className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-semibold hover:bg-indigo-700"
              >
                Pay ₹999 & Generate
              </button>
            </div>
            <div className="p-4 bg-gray-50 text-center border-t rounded-b-3xl">
