...
export default function ResumeFormatterPro({ app, isPro = false }) {
  useTrackAppUsage("resume-formatter");
  ...
  // ────────────────────────────────────────────────
  // IMPROVED FILE UPLOAD PARSER (2025 version)
  // ────────────────────────────────────────────────
  const handleFileUpload = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    if (!validTypes.includes(file.type)) {
      setError("Only PDF, DOC, DOCX, TXT allowed");
      return;
    }
    ...
    try {
      ...
      if (!text.trim()) {
        setError("No readable text extracted from file");
        return;
      }
      ...
      // ─── Name & Contact (first 10 lines) ────────────────────────────────
      for (let i = 0; i < Math.min(10, lines.length); i++) {
        const line = lines[i];
        if (!name && /^[A-Za-z\s\.,'-]{3,60}$/.test(line)) {
          name = line;
          continue;
        }
        if (/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}|linkedin|github|\+?\d{10}/.test(line)) {
          contact += (contact ? " | " : "") + line;
        }
      }

      // ─── Section detection & content collection ─────────────────────────
      lines.forEach((line) => {
        const lower = line.toLowerCase();

        // Skip name/contact lines
        if (line.includes(name) || line.includes(contact.split("|")[0]?.trim())) return;

        // Section headers
        if (/summary|profile|objective|professional summary|about/i.test(lower)) {
          currentSection = "summary";
          return;
        }
        if (/experience|work experience|employment|professional experience|career|position/i.test(lower)) {
          currentSection = "experience";
          return;
        }
        if (/education|degree|qualification|academic|school|college|university/i.test(lower)) {
          currentSection = "education";
          return;
        }
        if (/skill|skills|competenc|proficienc|expert|technolog|tool|stack|framework/i.test(lower)) {
          currentSection = "skills";
          return;
        }
        if (/project|projects|personal project|academic project|portfolio/i.test(lower)) {
          currentSection = "projects";
          return;
        }
        if (/achievement|award|certificat|accomplish|honor|publication/i.test(lower)) {
          currentSection = "achievements";
          return;
        }
        ...
        else if (currentSection === "experience") expLines.push(line);
        else if (currentSection === "education") eduLines.push(line);
        else if (currentSection === "skills") skillLines.push(line);
        else if (currentSection === "projects") projectLines.push(line);
        else if (currentSection === "achievements") achLines.push(line);
        else {
          // Fallback: early lines → summary, later → experience
          if (summaryLines.length < 6 && expLines.length < 3) {
            summaryLines.push(line);
          } else {
            expLines.push(line);
          }
  ...
  const generateATSResumeWithAI = async () => {
    if (!targetRole.trim() && !experience.trim()) {
      setError("Please enter target role or experience");
      return;
    }

    if (!isPro && generatedCount >= MAX_FREE_TRIALS) {
      setShowPayment(true);
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/resume-formatter/generate-ai`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          targetRole,
  ...
  const downloadAsTxt = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `Resume_${targetRole.replace(/\s+/g, "_") || "Professional"}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const downloadAsDoc = () => {
    ...
    const html = `
      ...
      <body style="font-family: Arial, sans-serif; margin: 40px 60px; line-height: 1.6; font-size: 11pt; color: #333;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="font-size: 28px; margin: 0; text-transform: uppercase; letter-spacing: 1.5px; font-weight: bold;">
            ${targetRole.toUpperCase() || "PROFESSIONAL RESUME"}
          </h1>
          <p style="margin: 12px 0; color: #555; font-size: 11pt;">
            ${targetCompany ? `Targeting: ${targetCompany} • ` : ""}
            ${industry ? `${industry} • ` : ""}${seniority}
          </p>
        </div>

        ${skills ? `
          <h2 style="font-size: 14pt; font-weight: bold; margin: 30px 0 10px 0;">SKILLS</h2>
          <p style="margin: 0; line-height: 1.5;">${skills}</p>
        ` : ""}

        ${experience ? `
          <h2 style="font-size: 14pt; font-weight: bold; margin: 30px 0 10px 0;">PROFESSIONAL EXPERIENCE</h2>
          ${experience.split('\n').filter(Boolean).map(l => `<p style="margin: 6px 0;">${l}</p>`).join('')}
        ` : ""}

        ${education ? `
          <h2 style="font-size: 14pt; font-weight: bold; margin: 30px 0 10px 0;">EDUCATION</h2>
          ${education.split('\n').filter(Boolean).map(l => `<p style="margin: 6px 0;">${l}</p>`).join('')}
        ` : ""}

        ${achievements ? `
          <h2 style="font-size: 14pt; font-weight: bold; margin: 30px 0 10px 0;">KEY ACHIEVEMENTS</h2>
          <ul style="padding-left: 20px; margin: 8px 0;">
            ${achievements.split('\n').filter(Boolean).map(l => `<li style="margin-bottom: 6px;">${l}</li>`).join('')}
          </ul>
        ` : ""}

        <p style="text-align:center; margin-top: 80px; color: #777; font-size: 9pt;">
          Generated by ATRact Resume Formatter Pro | ATS-Optimized | ${new Date().toLocaleDateString()}
        </p>
      </body>
      </html>
    `;
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `Resume_${targetRole.replace(/\s+/g, "_") || "Professional"}.doc`;
    a.click();
    URL.revokeObjectURL(url);
  };
      ...
      <div className="flex-1 ml-80 min-h-screen overflow-y-auto">
        <div className="max-w-5xl mx-auto p-6">
          ...
          {showPayment && (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white text-center rounded-t-2xl">
                  <h3 className="text-2xl font-bold">Unlock Unlimited Resumes</h3>
                </div>
                <div className="p-6 space-y-4">
                  <button className="w-full py-3 bg-indigo-600 text-white rounded-xl font-semibold">
                    Pay ₹99 - Format This Resume
                  </button>
                  <button className="w-full py-3 bg-gradient-to-r from-amber-400 to-orange-500 text-white rounded-xl font-semibold">
                    Upgrade to Pro - ₹499/month
                  </button>
                  <button
                    onClick={() => setShowPayment(false)}
                    className="w-full mt-4 text-gray-500 hover:text-gray-700 text-center text-sm"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div className="sticky top-0 z-10 bg-gray-100 pb-4 pt-2">
            <h1 className="text-3xl font-bold text-gray-900">{app?.name || "Resume Formatter Pro"}</h1>
            <p className="text-gray-600 mt-1">ATS-friendly resume formatting</p>
          </div>
          ...
          <div className="flex gap-3 mb-6 border-b border-gray-200 pb-2">
            <button
              ...
              className={`px-6 py-2.5 rounded-xl font-medium transition-all ${
                activeTab === "format" ? "bg-indigo-600 text-white shadow-sm" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
            >
              📝 Format Resume
            </button>
            <button
              ...
              disabled={!formattedResume}
              className={`px-6 py-2.5 rounded-xl font-medium transition-all ${
                activeTab === "results" ? "bg-indigo-600 text-white shadow-sm" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              } ${!formattedResume ? "opacity-50 cursor-not-allowed" : ""}`}
            >
              📄 Results
            </button>
          </div>
          ...
          {activeTab === "format" && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-2 space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input
                      ...
                      className="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    <input
                      value={targetCompany}
                      onChange={(e) => setTargetCompany(e.target.value)}
                ...
                {/* Upload Resume */}
                <div className="bg-white rounded-2xl shadow-sm p-6">
                  <h2 className="text-lg font-semibold mb-4">Upload Your Current Resume (Optional)</h2>
                  <div
                    onClick={() => !isProcessing && fileInputRef.current?.click()}
                  ...
                  >
                    ...
                    {isProcessing ? (
                      ...
                      <div className="py-8">
                        ...
                        <p className="font-medium text-green-700 text-lg">{uploadedFileName}</p>
                        <p className="text-sm text-gray-500 mt-2">Click to upload another file</p>
                      </div>
                    ) : (
                      <div className="py-8">
                        <div className="text-6xl mb-4 text-indigo-400">📄</div>
                        <p className="text-xl font-medium text-gray-700 mb-2">
                          Click here or drag & drop your resume
                        </p>
                        <p className="text-sm text-gray-500">
                          PDF, Word (.doc/.docx), or Text (.txt)
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
                  <div>
                    ...
                    <textarea
                      value={skills}
                      onChange={(e) => setSkills(e.target.value)}
                      placeholder="React, Node.js, JavaScript, AWS, Leadership..."
                      rows={2}
                  ...
                  <div>
                    ...
                    <textarea
                      value={experience}
                      onChange={(e) => setExperience(e.target.value)}
                      placeholder="Your roles, responsibilities, achievements..."
                      rows={6}
                      className="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">Key Achievements & Projects</label>
                    <textarea
                      value={achievements}
                      onChange={(e) => setAchievements(e.target.value)}
                      placeholder="Quantified accomplishments, awards, projects..."
                      rows={4}
                      className="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                    />
                  </div>
                ...
                <button
                  onClick={generateATSResumeWithAI}
                  disabled={isProcessing || (!targetRole.trim() && !experience.trim())}
                  className={`w-full py-4 rounded-xl text-lg font-semibold transition-all shadow-md ${
                    (targetRole.trim() || experience.trim()) && !isProcessing
                      ? "bg-indigo-600 text-white hover:bg-indigo-700"
                      : "bg-gray-200 text-gray-500 cursor-not-allowed"
                  }`}
                >
                  {isProcessing ? "Formatting..." : "Generate ATS-Optimized Resume"}
                </button>
              </div>
          ...
          {/* Results Tab - Clean layout like your 3rd image */}
          {activeTab === "results" && formattedResume && (
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-200 p-10">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-bold text-gray-900">ATS-Optimized Resume</h2>
                <div className="flex gap-3">
                  <button
                    onClick={copyToClipboard}
                    className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 rounded-xl text-sm font-medium flex items-center gap-2"
                  >
                    Copy
                  </button>
                  <button
                    onClick={downloadAsTxt}
                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium flex items-center gap-2"
                  >
                    TXT
                  </button>
                  <button
              ...
              {/* ────────────────────────────────────────────────
                   CLEAN LAYOUT LIKE YOUR 3rd IMAGE
              ──────────────────────────────────────────────── */}
              ...
              {skills && (
                <div className="mb-10">
                  <h2 className="text-xl font-bold uppercase mb-3">SKILLS</h2>
              ...
              {experience && (
                <div className="mb-10">
                  <h2 className="text-xl font-bold uppercase mb-3">PROFESSIONAL EXPERIENCE</h2>
              ...
              {education && (
                <div className="mb-10">
                  <h2 className="text-xl font-bold uppercase mb-3">EDUCATION</h2>
              ...
              {achievements && (
                <div className="mb-10">
                  <h2 className="text-xl font-bold uppercase mb-3">KEY ACHIEVEMENTS</h2>
              ...
              {/* Any remaining AI content */}
              <div className="space-y-4 text-gray-700 leading-relaxed">
                {formattedResume.split('\n').filter(l => l.trim() && !l.includes("ATS-Optimized")).map((line, i) => (
                  <p key={i}>{line}</p>
                ))}
              </div>

              <div className="text-center mt-12 text-sm text-gray-500">
                Generated by ATRact Resume Formatter Pro | {new Date().toLocaleDateString()}
              </div>
            </div>
          )}
        </div>
